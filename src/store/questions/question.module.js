...
const actions = {
  [FETCH_QUESTIONS]: async ({ commit, rootState }) => {
    return new Promise((resolve, reject) => {
      questionService.getQuestions(rootState.auth.token).then(
        res => {
  ...
  [ADD_QUESTION]: ({ commit, rootState }, payload) => {
    return new Promise((resolve, reject) => {
      questionService.addQuestion(rootState.auth.token, payload).then(
        res => {
  ...
  [EDIT_QUESTION]: ({ rootState }, payload) => {
    return new Promise((resolve, reject) => {
      questionService
        .editQuestion(rootState.auth.token, payload)
        .then(res => resolve(res), err => reject(err));
    });
  },
  [REMOVE_QUESTION]: ({ commit, rootState }, id) => {
    return new Promise((resolve, reject) => {
      questionService.removeQuestion(rootState.auth.token, id).then(
        res => {
          commit(SET_MESSAGE, "sucesso!");
          resolve(res);
        },
